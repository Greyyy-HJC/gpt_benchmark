...
sm_tag = "1HYP_GSRC_W90_k3_5" # NOTE
...
parameters = {
    ...
    "pf": [0,0,7,0],
    "p_2pt": [[x,y,z,0] for x in [0] for y in [0] for z in [7]], # 2pt momentum, should match pf & pi
...
#! GPT
sm_tag = "1HYP_GSRC_W90_k3_5_GPT" # NOTE
###################### loop over sources ######################
...
#! PyQUDA
sm_tag = "1HYP_GSRC_W90_k3_5_Pyquda" # NOTE
###################### loop over sources ######################
for pos in src_production:
    ...
    for iW, WL_indices in enumerate(W_index_list_CG):
        ...
        t0 = time.time()
        #TODO: try v2
        # proton_TMDs_down += [contract("pqgwtzyxjmcf, wtzyxmjfc -> pqgt", sequential_bw_joint_down_pyq, tmd_forward_prop.data).get()]
        # proton_TMDs_up += [contract("pqgwtzyxjmcf, wtzyxmjfc -> pqgt", sequential_bw_joint_up_pyq, tmd_forward_prop.data).get()]
